# -*- coding: utf-8 -*-
"""
Created on Tue Dec  3 16:42:22 2024
...
def labyrinthe_a_solution(labyrinthe, entree, sortie):
    file = deque([entree])  # File pour BFS
...
def mettre_a_jour_personnage(touche, perso, labyrinthe, items):
    ...
    if touche in mouvements:
        ...
        if 0 <= new_x < len(labyrinthe) and 0 <= new_y < len(labyrinthe[0]):
    ...
    while True:
        ...
        while True:
            ...
            if touche == "p":
                ...
                if termine:
                    score_total += 100 + len(items)  # +100 pour finir le niveau + items restants
                    niveau += 1
                    # nb_items += 1  # 
                    temps_par_niveau -= 15  # Réduit le temps alloué
                    break  # Passer au niveau suivant
